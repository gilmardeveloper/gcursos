	...
	public Sexo salvar(Sexo sexo) throws SexoExisteException {
		...
		if (sexo.getId() != null) {
			...
			if (eventoOnlineService.sexoExiste(buscarPor(sexo.getId()).getNome())) {
				throw new SexoExisteException(
						"para alterar essa opção é necessário antes excluir ou alterar as permissões dos eventos online");
			}

			if (eventoPresencialService.sexoExiste(buscarPor(sexo.getId()).getNome())) {
				throw new SexoExisteException(
						"para alterar essa opção é necessário antes excluir ou alterar as permissões dos eventos presenciais");
			}
		}
